...
def _caption_metrics(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    references = {
        record["sample"]["id"]: [
            _caption_metric_text(answer)
            for answer in record["sample"]["answers"]
        ]
        for record in records
    }
    candidates = {
        record["sample"]["id"]: [
            _caption_metric_text(record["prediction"]["text"])
        ]
        for record in records
    }
...
def _caption_metric_text(value: str) -> str:
    """Make text safe for line-oriented caption metric protocols.
    将文本转换为适合逐行描述指标协议的安全格式。
    """
