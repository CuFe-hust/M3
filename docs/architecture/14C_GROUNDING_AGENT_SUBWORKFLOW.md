...
模型接口必须先消除 YOLO resize / letterbox 的影响，再相对于实际 ROI 转换并
四舍五入到 0..999 整数。不得把模型预处理画布坐标直接暴露给最终 Qwen。

### 6.3 不生成标注图

Grounding 的最终 Qwen 只接收干净 ROI 和候选标注文本，不生成或输入带框标注图。
候选文本通过 `box_id` 关联类别与整数 xyxy 坐标。Qwen 根据原始问题、干净 ROI 和候选
文本进行语义筛选。
...
YOLO 候选文本只包含 `box_id`、具体类别、`roi_id` 和 ROI 局部 `0..999` 整数
`xyxy` JSON 坐标，不包含检测置信度。内部精确 ROI `[0,1]` 坐标只用于确定性
转换，不直接暴露给最终 Qwen。
...
模型侧两路框统一使用 ROI 局部 `0..999` 整数 `xyxy` JSON，只在最终后处理阶段
转回内部 ROI `[0,1]` 并进一步转为全图尺度。当前仓库
...
- Qwen 不看到 YOLO 置信度；
- YOLO 与 Qwen 自由框在模型输入/输出中都使用相对于 ROI 的 `0..999` 整数 `xyxy`；
