"""Counting geometry — owner-core/halo tiling, coordinate conversion.

计数几何 — owner-core/halo 切片与坐标换算。全部为纯确定性函数：不读文件
（crop_for_tile 接收已打开的图片）、不调用 Qwen/YOLO、不按数据集分支。
"""
...
def resize_dimensions_without_upscaling(
    width: int,
    height: int,
    model_max_side: int,
) -> tuple[int, int]:
...
def crop_for_tile(image: Image.Image, tile: TileSpec) -> Image.Image:
    """Crop normalized source pixels and resize only when model limits require
    it. Accepts an already-opened image; never opens files itself.
    裁剪规范化源像素，并仅在模型限制需要时缩放。接收已打开的图片；自身
    绝不打开文件。"""

    crop = image.crop(
        (
            tile.crop_global.left,
            tile.crop_global.top,
            tile.crop_global.right,
            tile.crop_global.bottom,
        )
    )
    if crop.size == (tile.model_input_width, tile.model_input_height):
        return crop
    return crop.resize(
        (tile.model_input_width, tile.model_input_height), Image.Resampling.LANCZOS
    )


# ── coordinate conversion / 坐标换算 ──────────────────────────────────────
...
def crop_pixel_to_global_pixel(
    crop_pixel: int,
    crop_start: int,
    source_length: int,
) -> tuple[int, bool]:
...
def ownership_tolerance_px(source_width: int, source_height: int) -> int:
    """Return the fixed small tolerance used only for boundary review
    candidates. 返回仅用于边界复核候选的固定小容差。"""
...
def within_owner_tolerance(
    ...
    return (
        ...
        and rect.top - tolerance <= y < rect.bottom + tolerance
    )


def is_near_core_boundary(
    rect: PixelRect,
    x: int,
    y: int,
    band_px: int,
) -> bool:
    """Return whether an accepted point is near an internal owner-core
    boundary. 返回接受点是否接近内部 owner core 边界。"""
...
def owner_core_prompt_norm(tile: TileSpec) -> list[int]:
    """Express owner core bounds in the tile crop's normalized coordinate
    system. 在切片 crop 的归一化坐标系中表示 owner core 边界。"""
...
def convert_local_point_to_global(
    ...
    *,
    ...
    crop_x = local_norm_to_crop_pixel(
        point.x, tile.crop_global.width, tile.model_input_width
    )
    crop_y = local_norm_to_crop_pixel(
        point.y, tile.crop_global.height, tile.model_input_height
    )
    global_x, x_was_clamped = crop_pixel_to_global_pixel(
        crop_x, tile.crop_global.left, tile.source_width
    )
    global_y, y_was_clamped = crop_pixel_to_global_pixel(
        crop_y, tile.crop_global.top, tile.source_height
    )
    core = tile.owner_core_global
    ...
    effective_tolerance = (
        ...
        else ownership_tolerance_px(tile.source_width, tile.source_height)
    )
    boundary_candidate = (
        not ownership_valid
        and within_owner_tolerance(core, global_x, global_y, effective_tolerance)
    )
    near_boundary = (
        is_near_core_boundary(core, global_x, global_y, boundary_band_px)
        or boundary_candidate
    )
