...
4. Dataset runs continue to use `DatasetRunner` via `get_adapter` / `RunStore` / `create_model` / `assemble_runtime` / `build_dataset_runner`. The dataset loop has exactly one implementation in `spacers_agent.commands.run_dataset.run_dataset`, shared by `main.py run-dataset` and the internal CLI; no caller may re-implement the loop.
...
8. 数据集运行继续使用 `DatasetRunner`（`get_adapter` / `RunStore` / `create_model` / `assemble_runtime` / `build_dataset_runner`）。数据集循环只有一份实现，位于 `spacers_agent.commands.run_dataset.run_dataset`，由 `main.py run-dataset` 与内部 CLI 共用；任何调用方不得重新实现循环。
