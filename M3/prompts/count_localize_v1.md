...
Set `agent_name` to `counting_agent`, keep `evidence` concise, and set `status` to `completed` when enumeration is complete.
...
将 `agent_name` 设为 `counting_agent`，`evidence` 保持简洁，枚举完成时将 `status` 设为 `completed`。
