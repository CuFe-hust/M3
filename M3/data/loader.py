"""Unified streaming dataset loader for all supported datasets.
统一的数据集流式读取入口。
"""
...
from data.downloader import DATASET_REPOS, DEFAULT_DATA_ROOT
...
# Supported evaluation targets read through this module.
# 本模块支持的评测读取目标。
DATASET_TARGETS = (
    "vrsbench_caption",
    "vrsbench_vqa",
...
def _resolve_data_root(data_root: str | Path | None) -> Path:
    """Resolve the local dataset root from argument, environment, or default.
    按参数、环境变量或默认值解析本地数据集根目录。
    """
...
def load_dataset(
    ...
    """Stream canonical samples for one dataset, optionally capped for smoke tests.
    流式读取一个数据集的统一样本，可按 limit 截断用于冒烟测试。
    """
    ...
    return _stream_dataset(dataset_name, _resolve_data_root(data_root), limit)


def _stream_dataset(dataset_name: str, root: Path, limit: int | None) -> Iterator[CanonicalSample]:
    """Yield canonical samples lazily and stop after the optional limit.
    惰性产出规范样本，并在达到可选上限后停止。
    """

    count = 0
    for sample in _iter_dataset(dataset_name, root):
        yield sample
        count += 1
...
def _iter_dataset(dataset_name: str, data_root: Path) -> Iterator[CanonicalSample]:
    """Dispatch one evaluation target to its streaming adapter.
    将评测目标分发到对应的流式适配器。
    """
...
def _main(argv: list[str] | None = None) -> None:
    """CLI entry for smoke-testing one dataset stream.
    用于冒烟测试单个数据集流量的命令行入口。
    """

    parser = argparse.ArgumentParser(description="Stream canonical dataset samples for smoke testing.")
    parser.add_argument("--dataset", choices=sorted(DATASET_TARGETS), required=True, help="Evaluation target to read.")
    parser.add_argument("--root", type=Path, default=None, help="Local dataset root; defaults to DATASET_ROOT or /data.")
    parser.add_argument("--limit", type=int, default=3, help="Maximum number of samples to print.")
    args = parser.parse_args(argv)
    count = 0
    for sample in load_dataset(args.dataset, args.root, args.limit):
        sample.validate()
        print(json.dumps(sample.serializable(), ensure_ascii=False, indent=2))
        count += 1
    if count == 0:
        raise RuntimeError(f"No samples loaded for {args.dataset}.")
