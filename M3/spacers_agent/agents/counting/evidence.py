...
def parse_count_answer(value: str) -> int:
    """Parse a non-negative integer from a count answer. / 从计数答案解析非负整数。"""
...
def recover_count_proposal_header(raw_response: str) -> int | None:
    """Recover integer answer from malformed JSON. / 从畸形 JSON 恢复整数答案。"""
...
def box_evidence(boxes: list[list[float]], target: str, image_id: str) -> list[VisualEvidence]:
    ...
    for raw_box in boxes:
        ...
        if box[0] >= box[2] or box[1] >= box[3]:
            continue
        evidence.append(VisualEvidence(label=target, box=box, confidence=0.9, image_id=image_id))
    return evidence
...
def is_tiny_border_fragment(box: list[int]) -> bool:
    ...
    cy = (box[1] + box[3]) / 2
    return (
        (box[0] == 0 and cx < 25) or (box[1] == 0 and cy < 25)
        or (box[2] == 999 and cx > 974) or (box[3] == 999 and cy > 974)
    )
...
def _same_count_observation(first: VisualEvidence, second: VisualEvidence) -> bool:
    ...
    if box_item.box is None or point_item.point is None:
        return False
    bc = [round((box_item.box[0] + box_item.box[2]) / 2), round((box_item.box[1] + box_item.box[3]) / 2)]
    return _point_distance(bc, point_item.point) <= 12


def merge_count_evidence(items: list[VisualEvidence]) -> list[VisualEvidence]:
    """Deduplicate count evidence (strict — adjacent vehicles remain distinct). / 去重计数证据（严格 — 相邻车辆保持独立）。"""
    merged: list[VisualEvidence] = []
    for candidate in items:
        dup_idx = next(
            (i for i, existing in enumerate(merged) if _same_count_observation(candidate, existing)), None,
        )
        if dup_idx is None:
            merged.append(candidate)
        elif (candidate.box is not None and merged[dup_idx].box is None) or candidate.confidence > merged[dup_idx].confidence:
            merged[dup_idx] = candidate
    return merged


def accepted_count_evidence(
    evidence: list[VisualEvidence], target: str, image_id: str,
) -> tuple[list[VisualEvidence], list[list[int]], int]:
    """Deduplicate, drop border fragments, emit accepted centres. / 去重、丢弃边界残片、输出接受中心。"""
    ...
    for item in merged:
        if item.box is not None:
            ...
            point = [round((item.box[0] + item.box[2]) / 2), round((item.box[1] + item.box[3]) / 2)]
        elif item.point is not None:
            point = list(item.point)
        else:
            dropped += 1
            continue
        raw_points.append(VisualEvidence(label=target, point=point, confidence=item.confidence, image_id=image_id))
...
def global_count_point(
    sample_id: str, target: str, evidence: VisualEvidence, index: int, width: int, height: int,
) -> GlobalPointObservation:
    """Convert normalized centre to whole-image provenance. / 将归一化中心转为整图来源记录。"""
    ...
    return GlobalPointObservation(
        ...
        target=target, source_tile_id="whole_image_overview", local_id=local_id,
        local_x_norm=x, local_y_norm=y, local_radius_norm=0,
        global_x_px=round(x * (width - 1) / 999), global_y_px=round(y * (height - 1) / 999),
        global_x_norm=x, global_y_norm=y, radius_px=0.0, confidence=evidence.confidence,
        ownership_valid=True, near_core_boundary=False, accepted=True,
