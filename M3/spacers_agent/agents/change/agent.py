...
class ChangeAgent:
    ...
    async def run(self, sample: UnifiedSample, context: AgentContext) -> AgentExecution:
        ...
        structured_prompt = (
            self._prompt.text
            + "\n\nReturn valid JSON only. Set agent_name to 'change_agent'; put the concise final answer in answer, "
        ...
        result = await self._client.complete_json(
            ...
            request_meta=RequestMeta(
                request_id=f"{sample.sample_id}:change_agent",
                request_hash=request_hash,
                prompt_version=self._prompt.version,
                sample_id=sample.sample_id,
                artifact_dir=context.artifact_dir / "change_agent",
            ),
        )
