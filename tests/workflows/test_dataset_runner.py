...
class _FakeVisualPlanner:
    """Return a strict v2 plan and record each planning call.
    返回严格的 v2 计划并记录每次规划调用。"""
...
def test_dataset_runner_task_none_is_internal_auto_task_mode(tmp_path: Path) -> None:
    ...
    default; without the v2 planner it fails at configuration time.
    task=None 是内部显式 auto-task 模式而非用户缺省；缺少 v2 规划器时在
    配置期失败。"""
    run_dir, _ = _create_run(tmp_path)
    runner = _runner(
        _FakeAdapter([]),
...
def test_visual_plan_explicit_entry_one_planner_call(tmp_path: Path) -> None:
    """An explicit sample gets one v2 planner call and one execution.
    显式样本获得一次 v2 规划调用与一次执行。"""
    ...
    # run_one got the rebuilt sample and the same v2 plan / run_one 收到
    # 重建样本与同一 v2 计划
...
def test_visual_planner_error_is_stable_failure(tmp_path: Path) -> None:
    ...
    planner = _FakeVisualPlanner(error=VisualTaskPlanError("LOW_CONFIDENCE"))
    stub = _StubSampleRunner()
    runner = _runner(
        _FakeAdapter([_sample("s0")]),
        stub,
...
def test_visual_plan_draft_entry_one_planner_call(tmp_path: Path) -> None:
    """The auto-task entry drafts through the v2 planner: one planner call
    per draft, materialization under the model-selected task, run_one with
    the v2 plan. auto-task 入口的 draft 经 v2 规划器处理：每条 draft 一次
    规划调用、按模型选定 task 物化、run_one 携带 v2 计划。"""
