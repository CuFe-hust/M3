...
from spacers_agent.agents.change.schemas import HarmonizationDecision, HarmonizationMetrics
from spacers_agent.settings import ChangeHarmonizationSettings


@dataclass(frozen=True)
class HarmonizationCandidate:
    """In-memory candidate retained only when the gate accepts it. / 仅门控通过时采用的内存候选。"""
...
class PairHarmonizer:
        ...
        if calibration and Path(calibration).is_file():
            try:
                ...
                if isinstance(p05, (int, float)):
                    self.settings = settings.model_copy(update={"min_pif_ratio": max(settings.min_pif_ratio, float(p05))})
    ...
    def run(self, t1: np.ndarray, t2: np.ndarray) -> HarmonizationCandidate:
        ...
        calibration_available = bool(self.settings.calibration_file and Path(self.settings.calibration_file).is_file())
        ...
        if int(np.count_nonzero(mask)) < self.settings.min_pif_pixels or ratio < self.settings.min_pif_ratio:
            decision = HarmonizationDecision(
                ...
                status="skipped",
                reason_codes=[*base_reasons, "SKIPPED_INSUFFICIENT_PIF", "RAW_FALLBACK_USED"],
                metrics=None,
                used_for_proposal=False,
            )
            return HarmonizationCandidate(raw1, raw2, mask, decision, {"sharpness_adjustment_used": False})
        ...
        for channel in range(3):
            ...
            stable = stable and all(np.isfinite([a1, b1, a2, b2])) and max(abs(a1), abs(a2)) <= self.settings.max_abs_gain and max(abs(b1), abs(b2)) <= self.settings.max_abs_offset
            transforms.append({"channel": float(channel), "t1_gain": a1, "t1_offset": b1, "t2_gain": a2, "t2_offset": b2})
            out1[..., channel] = lab1[..., channel] * a1 + b1
            out2[..., channel] = lab2[..., channel] * a2 + b2

        clipped_ratio = float(np.mean((out1 <= 0) | (out1 >= 255)) + np.mean((out2 <= 0) | (out2 >= 255))) * 0.5
        ...
        h1, h2, sharpness_used, sharpness_safe, sharpness_ratio_before, sharpness_ratio_after = _match_sharpness(color1, color2, self.settings)
        ...
        if not sharpness_safe:
            # An unsafe optional blur is rolled back locally; it does not invalidate safe color mapping.
            # 不安全的可选模糊仅局部回退，不使安全的颜色映射整体失效。
        ...
        elif self.settings.reject_when_pif_mad_worse and metrics.mad_pif_after > metrics.mad_pif_before * self.settings.max_pif_mad_degradation_ratio:
        ...
        return HarmonizationCandidate(h1 if used else raw1, h2 if used else raw2, mask, decision, summary)


def estimate_pif_mask(t1: np.ndarray, t2: np.ndarray, settings: ChangeHarmonizationSettings) -> np.ndarray:
    """Estimate PIF once from raw inputs; callers reuse this exact mask. / 仅从原图估计一次 PIF，调用方复用同一掩膜。"""
    ...
    low_diff = np.abs(cv2.GaussianBlur(g1, (ksize, ksize), 0) - cv2.GaussianBlur(g2, (ksize, ksize), 0))
    grad1 = cv2.magnitude(cv2.Sobel(g1, cv2.CV_32F, 1, 0), cv2.Sobel(g1, cv2.CV_32F, 0, 1))
    grad2 = cv2.magnitude(cv2.Sobel(g2, cv2.CV_32F, 1, 0), cv2.Sobel(g2, cv2.CV_32F, 0, 1))
...
def compute_metrics(raw1: np.ndarray, raw2: np.ndarray, out1: np.ndarray, out2: np.ndarray, mask: np.ndarray) -> HarmonizationMetrics:
    """Compute before/after metrics with the same raw-derived PIF mask. / 使用同一个原图 PIF 掩膜计算前后指标。"""
    ...
    return HarmonizationMetrics(
        ...
        corr_full_before=_corr(a, b), corr_full_after=_corr(x, y),
        corr_pif_before=_corr(a[selected], b[selected]), corr_pif_after=_corr(x[selected], y[selected]),
        pct_diff_gt20_before=float(np.mean(np.abs(a - b) > 20)),
        pct_diff_gt20_after=float(np.mean(np.abs(x - y) > 20)),
        lapvar_t1_before=_lapvar(raw1), lapvar_t2_before=_lapvar(raw2),
        lapvar_t1_after=_lapvar(out1), lapvar_t2_after=_lapvar(out2),
    )


def _match_sharpness(t1: np.ndarray, t2: np.ndarray, settings: ChangeHarmonizationSettings) -> tuple[np.ndarray, np.ndarray, bool, bool, float, float]:
    ...
    for sigma in np.linspace(0.3, settings.max_blur_sigma, 6):
        ...
        ratio = max(_lapvar(candidate), before[low]) / max(min(_lapvar(candidate), before[low]), 1e-6)
        if ratio < best_ratio:
