...
YOLOv5-OBB CSL 模型的延迟 ONNX Runtime 适配器。
"""
...
class YoloV5ObbOnnxModel:
    ...
    def __init__(self, weights: Path, classes: list[str]) -> None:
        try:
            ...
            from spacers_agent.agents.errors import OptionalDependencyMissingError  # noqa: PLC0415

            raise OptionalDependencyMissingError(
                "yolo", dependency="onnxruntime-gpu, numpy, opencv-python",
                install_hint="pip install -e '.[yolo-onnx]'",
            ) from exc
        self._cv2 = cv2
        self._np = np
        ...
        if len(inputs) != 1 or len(outputs) != 1:
            raise ValueError("YOLOv5-OBB ONNX model must expose exactly one input and one output")
        input_shape = list(inputs[0].shape)
        output_shape = list(outputs[0].shape)
        if input_shape[:2] != [1, 3] or input_shape[2:] != [1024, 1024]:
            raise ValueError(f"YOLOv5-OBB ONNX input must be [1, 3, 1024, 1024], got {input_shape!r}")
        expected_channels = 5 + len(classes) + 180
        if output_shape[-1] != expected_channels:
            raise ValueError(
                f"YOLOv5-OBB ONNX output must end with {expected_channels} channels, got {output_shape!r}"
            )
    ...
    def predict(
        ...
        *,
        ...
        """Run one fixed-size ONNX image and return an Ultralytics-like OBB payload.
        运行一张固定尺寸 ONNX 图像并返回类似 Ultralytics 的 OBB 载荷。
        """
        if imgsz != 1024:
        ...
        tensor = self._np.ascontiguousarray(prepared.transpose(2, 0, 1)[None]).astype(self._np.float32) / 255.0
        output = self._session.run(None, {self._input_name: tensor})[0][0]
        detections = self._nms(self._decode(output, conf), conf, iou, max_det)
        polygons = [self._to_source_polygon(box, ratio, padding, image.shape[1], image.shape[0]) for _, _, box in detections]
        obb = SimpleNamespace(
            xyxyxyxy=polygons,
            cls=self._np.asarray([class_id for class_id, _, _ in detections], dtype=self._np.float32),
            conf=self._np.asarray([score for _, score, _ in detections], dtype=self._np.float32),
        )
    ...
    def _letterbox(self, image: Any) -> tuple[Any, float, tuple[int, int]]:
        ...
        resized = self._cv2.resize(image, (round(width * ratio), round(height * ratio)), interpolation=self._cv2.INTER_LINEAR)
        ...
        bordered = self._cv2.copyMakeBorder(resized, top, bottom, left, right, self._cv2.BORDER_CONSTANT, value=(114, 114, 114))
    ...
    def _decode(self, prediction: Any, confidence: float) -> list[tuple[int, float, Any]]:
        ...
        angles = prediction[selected, 5 + class_count:].argmax(axis=1) / 180.0 * math.pi - math.pi / 2.0
        return [
            (int(class_id), float(score), self._np.concatenate((prediction[index, :4], [angle])))
            for index, class_id, score, angle in zip(selected, class_ids, scores, angles)
        ]

    def _nms(self, candidates: list[tuple[int, float, Any]], confidence: float, iou: float, max_det: int) -> list[tuple[int, float, Any]]:
        ...
        for class_id in sorted({item[0] for item in candidates}):
            ...
            rectangles = [
                ((float(box[0]), float(box[1])), (float(box[2]), float(box[3])), float(-box[4] * 180 / math.pi))
                for _, _, box in group
            ]
            indices = self._cv2.dnn.NMSBoxesRotated(rectangles, [score for _, score, _ in group], confidence, iou)
            if len(indices):
    ...
    def _to_source_polygon(self, box: Any, ratio: float, padding: tuple[int, int], width: int, height: int) -> Any:
        ...
        corners = self._np.asarray([
            (center_x + long_vector[0] + short_vector[0], center_y + long_vector[1] + short_vector[1]),
            (center_x + long_vector[0] - short_vector[0], center_y + long_vector[1] - short_vector[1]),
            (center_x - long_vector[0] - short_vector[0], center_y - long_vector[1] - short_vector[1]),
            (center_x - long_vector[0] + short_vector[0], center_y - long_vector[1] + short_vector[1]),
