...
在路由和专家工作流之间共享的可变单样本模型调用预算。纯数据契约，不依赖
任何完整配置对象，不构造 Agent；结构上满足 agents.base.CallBudget 协议。
"""
...
class CallBudget(BaseModel):
    ...
    def validate_usage(self) -> "CallBudget":
        ...
        if (
            self.qwen_calls_used > self.max_qwen_calls
            or self.deepseek_calls_used > self.max_deepseek_calls
        ):
...
class CallBudgetFactory:
    ...
    def create_for_sample(self, task: str) -> CallBudget:
        ...
        return CallBudget(
            max_qwen_calls=qwen_calls, max_deepseek_calls=deepseek_calls
        )


def make_budget_guard(
    budget: CallBudget, service: Literal["qwen", "deepseek"]
) -> Callable[[], None]:
