"""Local Transformers Qwen client with structured JSON, repair, cache, artifacts.

本地 Transformers Qwen 客户端：模型只加载一次；complete_json 结构化 JSON；
有界格式修复；请求哈希缓存；原始/解析产物、耗时与 token 记录。
本模块不导入旧运行时包，也不依赖 data / agents / application。
"""
...
# ── message translation and validation / 消息转换与校验 ─────────────────────
...
# ── repair and artifact helpers / 修复与产物辅助 ────────────────────────────
