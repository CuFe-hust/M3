...
本仓库是“面向太空智算的多模态遥感大模型应用探索”项目的新架构实现。当前长期架构位于 `new_structure`，迁移行为参考为 `try_yolo` 的锁定提交 `ec962eb87c3ad0b8c1502efcbd08db0daec48868`。旧分支只用于行为对照、Golden fixtures 与迁移审计，不再作为日常开发的第二套架构来源。
...
本版规则按照 `new_structure` 当前长期架构整理，重构迁移期的 Task 编号不再作为日常开发接口的一部分。
