...
## 当前状态（Task 00–33 完成，25.5–25.7 / 33.5 / 33.6 / 33.7 hardening 完成；新计划 Task 01–06 完成，并完成 06.5 workflow contract hardening。Task 07 Reporting 尚未开始）
...
- 单样本执行：`SampleRunner`（路由→attempt plan→Agent→产物→确定性评估→可选 judge→trace→状态；低置信度 TaskResolution 按最多 3 个候选构建去重 attempt plan，绝不跑所有 Agent；候选样本 model_copy 重建、不兼容稳定跳过；共享逐样本 CallBudget（可外部注入，贯穿 resolver 与 attempts）；确定性评估已接线：general_vqa/multiple_choice_vqa/scene_classification→VQA exact-match（vqa_evaluation.json）、counting/fine_grained_counting→计数（counting_evaluation.json，非 CountingResult 载荷 fail-closed）、grounding→轴对齐 IoU（grounding_evaluation.json，缺几何不伪造）、caption→逐样本候选+参考（caption_evaluation.json）；spatial_relation/change_qa/change_caption 无样本级指标（evaluation=None）；仅 general_vqa 走 judge 且经 asyncio.to_thread 不阻塞事件循环）
...
- 数据集执行：`DatasetRunner`（selection 固定顺序 adapter 稳定序→start_index→shard→sample_ids→limit（SHA256 分片）、resume 只跳过 succeeded 且只补缺失确定性评估/缺失或失败 VQA judge、单进程 asyncio 并发、fail-fast 取消不遗留 running、未启动样本记 `FAIL_FAST_NOT_STARTED`、`DatasetRunSummary` 计数强制闭合 total==succeeded+partial+failed+skipped；目录 `runs/<run_id>/tasks/<task>/samples/<sha256(sample_id)[:24]>`；`task=None` 是内部显式 auto-task mode（未来外部入口经 `DatasetRunOptions.auto_task=True` 选择），数据集 probe 经 `ArtifactWriter.write_dataset_probe` 独立写 `dataset_probe.json`——manifest.json 保持 RunManifest schema 可解析、绝不动态扩 schema）
...
**尚未实现**：reporting、application 与 CLI（`main.py`）。新计划 Task 07
（Reporting）尚未开始。请勿将 run-dataset 当作可用功能使用。
