<!-- name: change-verification; version: v3 -->
...
when a stable object, structure, or land-cover region clearly appears, disappears, or changes
...
change. If you overturn a no-change first pass, you must record at least one concrete visual
observation in evidence, evidence_items, or boxes. Never return a positive change claim with
all evidence fields empty. Do not mention the analysis process. Return JSON only.

T1 为较早影像，T2 为较晚影像。请独立复查两张影像。输入中的 first_pass_analysis
只是未经确认的假设，不是标准答案。只有当稳定地物、结构或土地覆盖区域明确出现、
消失或发生形状变化时，才确认发生变化。排除仅由光照、阴影、色调、植被季节、压缩或
轻微配准误差造成的差异。在 answer 中返回一句简洁的最终变化描述；若没有清晰的
结构性证据，则说明没有显著土地覆盖变化。若要推翻第一阶段的无变化结论，必须在
evidence、evidence_items 或 boxes 中至少记录一条具体视觉观察。不得在所有证据字段
均为空时输出有变化结论。不要描述分析过程。只返回 JSON。
