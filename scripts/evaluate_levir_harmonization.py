...
离线 LEVIR-CC 一致化评测与校准。使用当前 PairHarmonizer（agents.change）；
读取图对与标签只读；零 Qwen/DeepSeek 调用。单图对失败保持可见（写入
failed_pairs.json）而不丢失成功行。校准只输出 calibration.json，
绝不改写任何生产配置。导入边界：只 import 当前 change harmonizer/settings、
stdlib 与可选 numpy/cv2/PIL；绝不 import application runtime、models、
Judge/DeepSeek 或 legacy 包。
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate auditable LEVIR-CC temporal harmonization."
    )
    parser.add_argument("--dataset", choices=["LEVIR-CC"], default="LEVIR-CC")
    parser.add_argument("--root", type=Path, required=True)
    parser.add_argument(
        "--split", choices=["train", "validation", "val", "test"], required=True
    )
...
def main() -> int:
    ...
    names = sorted(
        {path.name for path in first_dir.glob("*.png")}
        & {path.name for path in second_dir.glob("*.png")}
    )
        ...
        try:
            ...
            row: dict[str, Any] = {
                ...
                "sharpness_adjustment_used": bool(
                    result.transform_summary.get("sharpness_adjustment_used", False)
                ),
                "has_change": None,
            }
            ...
            if metrics is not None:
                ...
                gains = [
                    ...
                    for key in ("t1_gain", "t2_gain")
                ]
                offsets = [
                    abs(float(item[key]))
                    for item in transforms
                    for key in ("t1_offset", "t2_offset")
                ]
                row["transform_gain_abs_max"] = max(gains, default=0.0)
                row["transform_offset_abs_max"] = max(offsets, default=0.0)
                row["sharpness_ratio_before"] = result.transform_summary.get(
                    "sharpness_ratio_before"
                )
                row["sharpness_ratio_after"] = result.transform_summary.get(
            ...
            if label_path.is_file() and metrics is not None:
                ...
                row.update(
                    _masked_change_metrics(
                        raw1, raw2, result.t1, result.t2, change_mask
                    )
                )
            rows.append(row)
    ...
    summary = {
        ...
        "split": split,
        "offset": args.offset,
        "requested_pairs": args.max_pairs,
    ...
    _atomic_write_json(
        args.output_dir / "grouped_summary.json",
        {
            "has_change": _summaries(
                [row for row in rows if row.get("has_change") is True]
            ),
            "no_change": _summaries(
                [row for row in rows if row.get("has_change") is False]
            ),
            "labels_available": any(row.get("has_change") is not None for row in rows),
        },
    )
    _atomic_write_json(args.output_dir / "failed_pairs.json", failures)
    if args.write_calibration:
        calibration = _calibration(rows, settings.version, split)
        _atomic_write_json(args.output_dir / "calibration.json", calibration)
...
def _masked_change_metrics(
    ...
    """Changed/unchanged MAD, background suppression, and change retention.
    变化/未变化 MAD、背景抑制与变化保留。"""

    before = np.abs(
        cv2.cvtColor(raw1, cv2.COLOR_RGB2GRAY).astype(np.float32)
        - cv2.cvtColor(raw2, cv2.COLOR_RGB2GRAY).astype(np.float32)
    )
    after = np.abs(
        cv2.cvtColor(out1, cv2.COLOR_RGB2GRAY).astype(np.float32)
        - cv2.cvtColor(out2, cv2.COLOR_RGB2GRAY).astype(np.float32)
    )
    unchanged = ~change
    ...
    return {
        "changed_mad_before": changed_before,
        "changed_mad_after": changed_after,
        "unchanged_mad_before": unchanged_before,
        "unchanged_mad_after": unchanged_after,
...
def _summaries(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Aggregate numeric metrics with n/mean/median/std and a deterministic
    bootstrap 95% CI. 以 n/mean/median/std 与确定性 bootstrap 95% CI 聚合
    数值指标。"""
...
def _bootstrap_ci(values: list[float]) -> list[float]:
    """Deterministic percentile bootstrap with a fixed seed.
    固定种子的确定性分位数 bootstrap。"""
...
def _calibration(
    rows: list[dict[str, Any]], version: str, split: str
) -> dict[str, Any]:
    """Output-only calibration summary; never rewrites production config.
    仅输出的校准汇总；绝不改写生产配置。"""
    ...
    def quantiles(key: str) -> dict[str, float]:
        ...
        return (
            {
                label: float(np.quantile(values, q))
                for label, q in (
                    ("p01", 0.01),
                    ("p05", 0.05),
                    ("p50", 0.5),
...
def _quantile_values(values: list[float]) -> dict[str, float]:
    ...
    return (
        {
            label: float(np.quantile(array, q))
            for label, q in (
                ("p01", 0.01),
                ("p05", 0.05),
                ("p50", 0.5),
