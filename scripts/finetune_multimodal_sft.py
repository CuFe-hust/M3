...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tuning-policy", default=None, choices=("lora_only", "projector_only", "lora_plus_projector", "full_language_lora"))
    parser.add_argument("--lora-rank", "--lora_rank", type=int)
    parser.add_argument("--lora-alpha", "--lora_alpha", type=int)
    ...
    parser.add_argument("--connector-lr", "--connector_lr", "--merger-lr", "--merger_lr", dest="connector_lr", type=float)
    ...
    parser.add_argument("--gradient-accumulation", "--gradient-accumulation-steps", "--gradient_accumulation_steps", dest="gradient_accumulation", type=int)
    parser.add_argument("--gradient-checkpointing", "--gradient_checkpointing", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--epochs", "--num-train-epochs", "--num_train_epochs", dest="epochs", type=int)
    ...
    parser.add_argument("--eval-steps", "--eval_steps", type=int)
    parser.add_argument("--save-steps", "--save_steps", type=int)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    effective_tuning_policy = args.tuning_policy or (
        "lora_only" if args.data_profile == "grounding" else "lora_plus_projector"
    )
    effective_connector_lr = (
        args.connector_lr
        if args.connector_lr is not None
        else (0.0 if args.data_profile == "grounding" else 1e-5)
    )
    effective_lora_rank = (
        args.lora_rank
        if args.lora_rank is not None
        else (32 if args.data_profile == "grounding" else 64)
    )
    effective_lora_alpha = (
        args.lora_alpha
        if args.lora_alpha is not None
        else (64 if args.data_profile == "grounding" else 128)
    )
    effective_gradient_accumulation = (
        args.gradient_accumulation
        if args.gradient_accumulation is not None
        else (16 if args.data_profile == "grounding" else 1)
    )
    effective_epochs = (
        args.epochs
        if args.epochs is not None
        else (2 if args.data_profile == "grounding" else 1)
    )
    effective_eval_steps = (
        args.eval_steps
        if args.eval_steps is not None
        else (34 if args.data_profile == "grounding" else 100)
    )
    effective_save_steps = (
        args.save_steps
        if args.save_steps is not None
        else (34 if args.data_profile == "grounding" else 0)
    )
    try:
        ...
        selected_policy = replace(
            TuningPolicy.from_name(effective_tuning_policy),
        ...
        training_result = trainer.fit(
            ...
            config=TrainingConfig(
                ...
                connector_lr=effective_connector_lr,
        ...
        result["final_adapter"] = str(training_result.final_adapter_path) if training_result.final_adapter_path else None
